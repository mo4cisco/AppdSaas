...
install="$(curl -s --location --request GET "${URL}/zero/v1beta/install/installCommand?sudo=false&multiline=false&application=${APP_NAME}&accessKey=fillmein&serviceUrl=${URL}" \
--header "Authorization: Bearer ${temp}" --data-raw '')"
